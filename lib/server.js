  ...
  resolveFqns: function(client, fqns) {
    ...
    for (var i=0; i<fqns.length; i++) {
      ...
      if (typeof client.you === 'object') {
        ...
        console.log('[YOUJS]: invalid client.you object', client.you);
      }
    }
  },
  refreshGroupFqns: function(group, client) {
    if (client && client.you && group && group.you) {
      if (group.members[client.clientId]) {
              ...
              if (isLastPiece === true) {
                activePathObj[piece] = function() {
                  for (var clientId in group.members) {
                    ...
                    var argMap = arguments;
                    client.ready(function() {
                      var fn = util.getFnFromFqn(this.client.you, this.fqn);
                      if (typeof fn === 'function') {
                        var args = [];
                        for (var argNum in this.argMap) args.push(this.argMap[argNum]);
                        fn.apply(client, args);
                      } else {
                        console.log('[YOUJS]: client ' + this.client.clientId + ' has no function ' + this.piece + ' (fqn: ' + this.fqn + '). Clients you space is ', this.client.you);
                      }
                    }.bind({client: client, piece: this.piece, fqn: this.fqn, argMap: argMap}));
                  }
        ...
        console.log('[YOUJS]: will not refresh group functions because client ' + client.clientId + ' is not member of group ' + group.name);
      }
    } else {
      console.log('[YOUJS]: cannot refresh group functions because client or group is invalid');
    }
  }
}
...
youjs.prototype = {
  ...
  init: function(server) {
    ...
    io.sockets.on('connection', function (socket) {
      ...
      socket.on('disconnect', function() {
        console.log('[YOUJS]: client with clientId ' + socket.id + ' disconnected.');
      ...
      socket.on('fqns', function(fqns) {
        ...
        if (fqns != null && fqns instanceof Array) {
          ...
          console.log('[YOUJS]: invalid or empty fqns passed. Must be an Array');
        }
      });
      ...
      socket.on('executed', function(opts) {
        ...
        if (opts && opts.id !== null && opts.args !== null) {
          ...
          if (client.cbMap[opts.id]) {
            ...
            // TODO: figure out a way to execute a callback multiple times without storing the callback function forever in memory
      ...
      socket.on('execute', function(opts) {
        if (opts && opts.fqn !== null && opts.args !== null) {
          ...
          if (typeof fn === 'function') {
            ...
            console.log('[YOUJS]: could not resolve fqn ' + opts.fqn + ' of group ' + opts.grp);
          }
        }
      });
    });
  ...
  sendFunctions: function(clientId) {
    ...
    if (self.clients[clientId]) {
      ...
      console.log('[YOUJS]: Cannot send server functions to client ' + clientId + ' because the client is not registered');
    }
  },
    ...
    if (!this.groups[groupName]) {
      var group = {
        ...
        join: function(clientId) {
          if (self.clients[clientId]) {
            console.log('[YOUJS]: client with clientId ' + clientId + ' is joining group ' + groupName);
  ...
  serveClient: function(server) {
    ...
    if ('undefined' !== typeof server) {
          ...
          fs.exists(clientPath, function(exists) {
            if (exists) {
              fs.readFile(clientPath, function(err, content) {
                if (err) {
                  console.log('[YOUJS]: error reading file ' + clientPath);
                  res.writeHead(500);
                  res.end();
                } else {
                  try {
      ...
      console.log('[YOUJS]: http server not initialized');
    }
  }
...
module.exports = {
  createServer: function(server) {
    return new youjs(server);
  },
  createClient: function(uri) {
    return client(uri);
  }
}
